  ...
  <div>
    ...
    <div class="section">
      ...
      <div class="row">
        ...
        <button @click="loadBorrowed">查询当前借书</button>
      </div>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="book in borrowedBooks" :key="book.id">
              ...
              <td>
                <button class="btn-return" @click="doReturn(book.book_no)">还书</button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <div class="section">
      ...
      <div class="row">
        ...
        <button @click="doReturn(book_no)">确认还书</button>
      </div>
...
async function doReturn(bookNo) {
  if (!card_no.value || !bookNo) {
    msg.ok = false
  ...
  try {
    ...
    msg.ok = false
    msg.text = error.response?.data?.detail || '还书失败，请重试'
  }
}
</script>
...
.btn-return { background: #67c23a; padding: 5px 14px; font-size: 13px; }
.btn-return:hover { background: #85ce61; }
